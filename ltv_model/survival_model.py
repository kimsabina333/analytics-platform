...
class SurvivalModel:
    ...
    def __init__(self, hidden_dim=16, 
                 ...
                 device='cpu', num_features=('upsell_amount', 'first_amount'), 
                 payment_col: str='paid_count', 
                 payment_target_col: str='amount',
        ...
        self.payment_target_col = payment_target_col
        self.payment_model = HistGradientBoostingRegressor()
        self.preprocessor = Preprocessor(num_features)
    ...
    def fit(self, X_train, T_train, c_train, payment_data, X_test=None, T_test=None, c_test=None):

        self.fit_processor(X_train)

        X_train_processed = self.preprocess(X_train)
        X_payment = X_train_processed.merge(payment_data[[self.payment_col, self.payment_target_col]], left_index=True, right_index=True, how='inner')
        X_test_processed  = self.preprocess(X_test) if X_test is not None and X_test.shape[0] > 0 else None
        ...
        self.fit_payment(X_payment.drop(columns=[self.payment_target_col]), X_payment[self.payment_target_col])
    ...
    def fit_model(self, X_train, T_train, c_train, X_test=None, T_test=None, c_test=None):
        ...
        if (X_test is None or X_test.shape[0] == 0) and (T_test is None or T_test.shape[0] == 0) and (c_test is None or c_test.shape[0] == 0):
            self.test_data = None
        else:
        ...
        for epoch in range(1, self.n_epochs+1):
            ...
            if self.show_training_plots:
                ...
                if test_loader is None:
                    ax.set_title(f'Train: {round(train_loss_list[-1], 3)}')
                    ax.plot(np.arange(1, len(train_loss_list)+1), train_loss_list)
                    ax.set_xticks(np.arange(1, self.n_epochs+1))
                else:
    ...
    def survival_function(self, X=None, method='expectation', n_period=6):
        ...
        for t in range(0, n_period):
            ...
            if method == 'expectation':
                ...
                S_hat = torch.pow(alpha / (alpha + beta), torch.pow(t + 1, gamma))
            survival_dict[f'S{int(t)}'] = S_hat

        self.survival_hat = pd.DataFrame(survival_dict, index=index)
        self.hidden_params = pd.DataFrame({'alpha': alpha, 'beta': beta, 'gamma':gamma}, index=index)
    ...
    def ltv_calc(self, X, n_periods: Optional[Union[int, np.ndarray, list]] = 6):
        ...
        max_period = int(np.max(n_periods))
        ...
        X_pay = pd.DataFrame(
                        np.concatenate(
                            [
                            np.repeat(X.values, max_period, axis=0),
                            np.repeat(paid_count.reshape(1,-1), X.shape[0], axis=0).flatten().reshape(-1,1)
                            ],
                            axis=1
                            ),
                        columns=X.columns.tolist() + [self.payment_col], 
    ...
    def save(self, directory_path='./ltv_model_weights', model_path='bdw_weights.pth', config_path='config.joblib'):
        os.makedirs(directory_path, exist_ok=True)
        torch.save(self.model.to('cpu').state_dict(), os.path.join(directory_path, model_path))
        ...
        config = {
            ...
            'payment_model_config': {
                ...
                'payment_target_col': self.payment_target_col,
            },
        ...
        joblib_path = os.path.join(directory_path, config_path)
        joblib.dump(config, joblib_path)
        return self
    
    @classmethod
    def load(cls, directory_path='./ltv_model_weights', model_path='bdw_weights.pth', config_path='config.joblib'):
        """
        ...
        instance = cls(
            ...
            num_features=bdw_config['num_features'],
            lr=bdw_config['lr'],
        ...
        torch_path = os.path.join(directory_path, model_path)
        ...
        instance.payment_target_col = payment_config['payment_target_col']
