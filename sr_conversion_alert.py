...
import yaml
q_map_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'idata/sr_alert_q_map.yaml')
...
sr_model = ModelLoader(coords=coords)
...
def sr_alert(**kwargs):
    ...
    def compute_sr(col, val, min_cnt=50, min_days=3, q=0.05):
        ...
        sr_fact = data.query(query_str).groupby('date').apply(
            lambda x: x.success.sum() / x.cnt.sum()
        )
    ...
    for val in offers:
        ...
        if is_alert:
            text = f'\n📉 Success rate for *offer: {val}* is below CI.'
    ...
    for val in mids:
        ...
        make_plot(sr_dist, sr_fact, q=q, fig_path=fig_path, save=True)
        if is_alert:
            text = f'\n📉 Success rate for *mid: {val}* is below CI.'
            send_slack_image_alert(channel_id=slack_test_channel_id, text=text, file_url=fig_path, title=val)
        else:
    ...
    for val in utm_sources:
        ...
        make_plot(sr_dist, sr_fact, q=q, fig_path=fig_path, save=True)
        if is_alert:
            text = f'\n📉 Success rate for *utm_source: {val}* is below CI.'
            send_slack_image_alert(channel_id=slack_test_channel_id, text=text, file_url=fig_path, title=val)
        else:
            print(f'utm_source={val}: SR normal, no alert')

    
    # цикл mid + payment_method
    for mid_val, pm_val in mid_pm_combos:
        ...
        make_plot(sr_dist, sr_fact, q=q, fig_path=fig_path, save=True)
        if is_alert:
            text = f'\n📉 Success rate for *mid: {mid_val} / payment_method: {pm_val}* is below CI.'
    ...
    for mid_val, cb_val in mid_cb_combos:
        ...
        make_plot(sr_dist, sr_fact, q=q, fig_path=fig_path, save=True)
        if is_alert:
            text = f'\n📉 Success rate for *mid: {mid_val} / card_brand: {cb_val}* is below CI.'
...
with DAG(
    ...
    description="[Alert] Last day TTP -> SUB conversion",
    schedule="1 23 * * *",
    start_date=datetime(2023, 11, 14),
    on_failure_callback=telegram_task_failed_alert,
    catchup=False,
    tags=["Funnel Conversion", "Alert", "Payment"],
) as dag:
